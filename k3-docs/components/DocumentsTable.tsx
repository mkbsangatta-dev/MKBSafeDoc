...
export default function DocumentsTable({
  ...
  const [preview, setPreview] = useState<{ url: string; name: string; kind: "image" | "pdf" | "other" } | null>(
    null
  );
  ...
  async function handlePreview(doc: DocumentRow) {
    ...
    if (error || !data) {
      alert("Gagal membuka pratinjau. Coba lagi.");
      return;
    }
    setPreview({ url: data.signedUrl, name: doc.file_name, kind: fileKind(doc.file_name) });
  }
  ...
  return (
    <div>
      ...
      {preview && (
        ...
        >
          <div
            className="bg-white rounded-sm w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-ink/10">
              <p className="text-sm font-medium text-ink truncate pr-4">{preview.name}</p>
              <div className="flex items-center gap-3 shrink-0">
                <a
                  href={preview.url}
                  target="_blank"
                  rel="noreferrer"
                  className="focus-ring text-xs text-steel hover:underline"
                >
                  Buka di tab baru
                </a>
                <button
                  onClick={() => setPreview(null)}
                  className="focus-ring text-ink/50 hover:text-ink text-lg leading-none"
            ...
            <div className="flex-1 overflow-auto bg-ink/5">
              ...
              {preview.kind === "pdf" && (
                <iframe src={preview.url} title={preview.name} className="w-full h-[75vh] border-0" />
              )}
              {preview.kind === "other" && (
                <div className="py-16 text-center">
                  <p className="text-sm text-ink/60 mb-3">
                    Jenis file ini tidak bisa ditampilkan langsung.
                  </p>
                  <a
                    href={preview.url}
                    target="_blank"
                    rel="noreferrer"
                    className="focus-ring text-sm text-steel hover:underline"
                  >
                    Unduh untuk membuka
                  </a>
                </div>
